...
def chunks_to_docs_wrapper(row: dict) -> Document:
  """
  Converts a row from a DataFrame into a Document object suitable for ingestion into MongoDB.
...
def main():
  ...
  if pdf is not None:
    ...
    # Create knowledge base in MongoDB
    # Initialize model
    model_4db = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")

    # Create vector store
    knowledge_base = collection  # Use MongoDB collection as the knowledge base
    ...
    if user_question:
      s3time =  time.time()
      result_chunks = knowledge_base.find()  # Perform similarity search in MongoDB
      s4time = time.time()
